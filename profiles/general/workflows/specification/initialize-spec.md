...
OTHERWISE, if you were given a Linear ticket id that matches the regexp `^[A-Z]{3}-\d+$`, then retrieve the contents of the ticket by using the command `linearis issues read <linear-ticket-id>` where `<linear-ticket-id>` is the specified ticket id.
