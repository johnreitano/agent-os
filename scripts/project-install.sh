...
install_claude_code_commands_with_delegation() {
    ...
    while read file; do
        ...
        if [[ "$file" == commands/*/multi-agent/*.md ]] || [[ "$file" == commands/*/*.md ]]; then
            local source=$(get_profile_file "$EFFECTIVE_PROFILE" "$file" "$BASE_DIR")
            if [[ -f "$source" ]]; then
...
install_claude_code_commands_without_delegation() {
    ...
    while read file; do
        # Process single-agent command files
        if [[ "$file" == commands/*/single-agent/*.md ]] || [[ "$file" == commands/*/*.md ]]; then
            local source=$(get_profile_file "$EFFECTIVE_PROFILE" "$file" "$BASE_DIR")
            if [[ -f "$source" ]]; then
                # Only install non-numbered files (e.g., plan-product.md, not 1-product-concept.md)
                local filename=$(basename "$file")
                if [[ ! "$filename" =~ ^[0-9]+-.*\.md$ ]]; then
                    local phase_mode=$([[ "$file" == commands/*/*.md ]] && echo "" || echo "embed")
                    local cmd_name=$(echo "$file" | cut -d'/' -f2)
...
install_agent_os_commands() {
    ...
    while read file; do
        ...
        if [[ "$file" == commands/*/single-agent/* ]] || [[ "$file" == commands/*/*.md ]]; then
            local source=$(get_profile_file "$EFFECTIVE_PROFILE" "$file" "$BASE_DIR")
            if [[ -f "$source" ]]; then
